...
BATCH_SIZE    = 40        # titoli per chiamata API (bilancia velocità/costo)
MAX_RETRIES   = 3
...
# Parole comuni italiane — se il testo ne contiene, è già in italiano
_IT_WORDS = {
    "il","la","lo","le","gli","un","una","di","da","in","con","su","per",
    "tra","fra","che","chi","come","quando","dove","questo","questa","quello",
    "della","dello","degli","delle","del","dal","alla","allo","agli","alle",
    "al","nei","nelle","negli","una","uno","è","non","si","ha","ho","ma",
    "anche","tutto","tutti","dopo","prima","durante","oggi","domani","sera",
    "film","serie","puntata","episodio","stagione","documentario","notizie",
    "sport","calcio","partita","campionato","speciale","diretta","replica",
}

def is_italian(text: str) -> bool:
    """Ritorna True se il testo sembra già scritto in italiano."""
    if not text or len(text) < 4:
        return True   # testi cortissimi non vale la pena tradurre
    words = set(text.lower().split())
    # Se almeno 2 parole italiane comuni sono presenti → già italiano
    return len(words & _IT_WORDS) >= 2
...
def translate_programmes(client: anthropic.Anthropic, programmes: list) -> dict:
    """
    Raccoglie tutti i titoli/descrizioni unici, salta quelli già in italiano,
    traduce il resto in batch. Restituisce {testo_originale: testo_tradotto}.
    """
    # Raccogli testi unici
    to_translate = set()
    ...
    # Filtra quelli già in italiano
    translations = {}
    needs_translation = []
    for text in to_translate:
        if is_italian(text):
            translations[text] = text   # già italiano, nessuna chiamata API
        else:
            needs_translation.append(text)

    skipped = len(to_translate) - len(needs_translation)
    print(f"[*] Testi unici totali: {len(to_translate)} | già italiani (saltati): {skipped} | da tradurre: {len(needs_translation)}")
    ...
    for i in range(0, len(needs_translation), BATCH_SIZE):
        ...
        # Piccola pausa solo ogni 5 batch per evitare rate-limit
        if batch_num % 5 == 0:
            time.sleep(2)
